...
public final class MyStrategy implements Strategy {
    ...
    private static final double LOW_HP_FACTOR = 0.66D;
    private static final double LOWLOW_HP_FACTOR = 0.33D;
    ...
    public void move(Wizard self, World world, Game game, Move move) {
        ...
        if (game.isSkillsEnabled()) {
            if (self.getLevel() > levelup) {
                //SkillType myskill = new SkillType(MAGICAL_DAMAGE_BONUS_AURA_1);
                //public static final SkillType MAGICAL_DAMAGE_BONUS_AURA_1
                move.setSkillToLearn(move.getSkillToLearn());
                //System.out.println("levelup razgildyai");
                //    self.getSkills();
                this.levelup++;
            }
        }
        ...
        move.setStrafeSpeed(random.nextBoolean() ? game.getWizardStrafeSpeed() * 2 : -game.getWizardStrafeSpeed() * 2);
        // Если осталось мало жизненной энергии, отступаем к предыдущей ключевой точке на линии.
        if (self.getLife() < self.getMaxLife() * LOWLOW_HP_FACTOR) {
    ...
    private void initializeStrategy(Wizard self, Game game) {
        if (random == null) {
            ...
            waypointsByLane.put(LaneType.MIDDLE, new Point2D[]{
                    new Point2D(200.0D, mapSize - 500.0D),
                    new Point2D(400.0D, mapSize - 600.0D),
                    new Point2D(800.0D, mapSize - 800.0D),
                    new Point2D(1200.0D, mapSize - 1200.0D),
                    new Point2D(1450.0D, mapSize - 1600.0D),
                    new Point2D(1850.0D, mapSize - 2000.0D),
                    new Point2D(2250.0D, mapSize - 2400.0D),
                    new Point2D(2650.0D, mapSize - 2800.0D),
                    new Point2D(3050.0D, mapSize - 3200.0D),
//                    random.nextBoolean()
...
//                    new Point2D(800.0D, mapSize - 800.0D),
            ...
            waypointsByLane.put(LaneType.TOP, new Point2D[]{
                    new Point2D(mapSize * 0.03, mapSize * 0.9),
                    new Point2D(mapSize * 0.05, mapSize * 0.8),
                    new Point2D(mapSize * 0.05, mapSize * 0.7),
                    new Point2D(mapSize * 0.05, mapSize * 0.6),
                    new Point2D(mapSize * 0.05, mapSize * 0.5),
                    new Point2D(mapSize * 0.05, mapSize * 0.45),
                    new Point2D(mapSize * 0.05, mapSize * 0.4),
                    new Point2D(mapSize * 0.08, mapSize * 0.35),
                    new Point2D(mapSize * 0.08, mapSize * 0.3),
                    new Point2D(mapSize * 0.08, mapSize * 0.25),
                    new Point2D(mapSize * 0.08, mapSize * 0.17),
                    new Point2D(mapSize * 0.2, mapSize * 0.17),
                    new Point2D(mapSize * 0.25, mapSize * 0.05),
                    new Point2D(mapSize * 0.55, mapSize * 0.05),
                    new Point2D(mapSize * 0.75, mapSize * 0.05),
...
///
//new Point2D(mapSize*0.03, mapSize*0.95),
